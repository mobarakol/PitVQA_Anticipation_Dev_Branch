...
class PitVQAGen(nn.Module):
    ...
    def forward(self, image, qa_inputs_ids, qa_att_mask):
        ...
        # Add video features to beginning of text sequence
        video_embeds = video_embeds.unsqueeze(1)  # [batch_size, 1, 768]
        
        # Concatenate video and text features
        fused_embeds = torch.cat([video_embeds, text_features], dim=1)  # [batch_size, seq_len+1, 768]
        
        # Create attention mask for fused features
        video_atts = torch.ones(video_embeds.size()[:-1], dtype=torch.long, device=video_embeds.device)
        fused_att_mask = torch.cat([video_atts, qa_att_mask], dim=1)  # [batch_size, seq_len+1]
...
def train(args, train_dataloader, model, criterion, optimizer, epoch, tokenizer, device):
    ...
    for i, (images, questions, answers) in enumerate(train_dataloader, 0):
        ...
        # Add a label for the video token at the beginning (should be ignored)
        video_labels = torch.full((labels.size(0), 1), -100, dtype=labels.dtype, device=device)
        labels = torch.cat([video_labels, labels], dim=1)  # [batch_size, seq_len+1]
...
def batch_greedy_search(images, questions, model, tokenizer, max_length, device):
    ...
    with torch.no_grad():
        ...
        # Prepare model inputs - account for video token at the beginning
        ...
        for _ in range(max_length):
            ...
            # Since video token is at position 0, we need to account for it in logits
            last_valid_logits = logits[batch_indices, valid_lengths, :]  # Note: no -1 because video token shifts everything

...
class PitVQAGen(nn.Module):
    def __init__(self, peft_config=None):
        ...
        # Add projection layer to match expected fusion dimension
        self.video_proj = nn.Linear(512, 1024)  # X-CLIP video features are 512D, project to 1024D for fusion (Qwen3-0.6B hidden size)

        # Cross-attention fusion module
        self.cross_attention_fusion = RecurrentCrossAttentionFusion(embed_dim=1024, num_heads=16, steps=3)

...
def train(args, train_dataloader, model, criterion, optimizer, epoch, tokenizer, device):
    ...
    for i, (images, questions, answers) in enumerate(train_dataloader, 0):
        ...
        scores, answer_GT_ID_sorted, decode_lengths, alphas, sort_ind = model(question_inputs.to(device), images.to(device), answers_GT_ID, answers_GT_len)  # [bs, ans_len, vocab]
...
def get_arg():
    ...
    parser.add_argument('--question_len',   type=int, default=50, help='sequence length for question')
    ...
    parser.add_argument('--alpha_c', type=float, default=1.0, help='regularization parameter for doubly stochastic attention, as in the paper.')
...
if __name__ == '__main__':
    ...
    emb_dim = 300  # follow the surgical-VQA paper
    encoder_layers = 6
    decoder_layers = 6
    dropout = 0.1
    n_heads = 8
    ...
    model = VisualBertSentence(vocab_size=len(tokenizer), embed_dim=emb_dim, encoder_layers=encoder_layers,
                               decoder_layers=decoder_layers,
                               dropout=dropout, n_heads=n_heads, answer_len=args.answer_len)

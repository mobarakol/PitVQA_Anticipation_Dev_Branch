...
# os.environ['HF_HOME'] = '/SAN/medic/surgicalLLM/llamma-models'
...
# os.environ['HF_HOME'] = 'SAN/medic/surgicalLLM/Qwen-model'
# hf_home = os.path.expanduser(
#     os.getenv("HF_HOME", os.path.join(os.getenv("XDG_CACHE_HOME", "~/.cache"), "huggingface"))
# )
...
def analyze_category_performance(all_results):
    """Analyze performance across different categories for all system messages"""
    print(f"\n{'='*80}")
    ...
    all_categories = set()
    for result in all_results:
        all_categories.update(result['categorical_metrics'].keys())
    
    for category in sorted(all_categories):
        print(f"\nCategory: {category}")
        print(f"{'System Message':<20} {'Count':<8} {'BLEU':<10} {'Rouge1':<10} {'Meteor':<10}")
        print("-" * 65)
        
        for result in all_results:
            if category in result['categorical_metrics']:
                cat_metrics = result['categorical_metrics'][category]
                print(f"{result['system_message_name']:<20} "
                      f"{cat_metrics['count']:<8} "
                      f"{cat_metrics['bleu']:<10.4f} "
                      f"{cat_metrics['rouge1']:<10.4f} "
                      f"{cat_metrics['meteor']:<10.4f}")
...
def main():
    ...
    # Configuration
    max_samples = None  # Set to None for full dataset, or specify number for tessting
    output_directory = "/SAN/medic/surgicalLLM/content/zeroshot-test/results/llama_system_message_results"  # Specify output directory
    ...
    for msg_name, msg_text in SYSTEM_MESSAGES.items():
        try:
            ...
            save_results_to_file(result, f"llama_results_{msg_name}_{timestamp}.json", output_directory)
            
            # Optional: Clear memory between tests (if needed for memory management)
            # clear_memory()
    ...
    # Save all results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    save_results_to_file(all_results, f"llama_all_system_message_results_{timestamp}.json", output_directory)
    
    # Compare results
    if all_results:
        compare_system_messages(all_results)
        analyze_category_performance(all_results)
